...
def getTweetData(searchTweet, tweetLimit,userData):

    #Call the connect funciton to connect to the Twitter API
    auth = connectTwitter()
    ...
    tagList = [] #Total List of Hashtags
    currTagList = [] #List of Hashtags
    urlList = [] #Total list of URLs in tweets
    ...
    for tweet in tweepy.Cursor(api.search,q=searchTweet, lang="en",tweet_mode='extended').items(tweetLimit):
        ...
        ###    print(attrH['text'])
            tagList.append(attrH['text'].lower()) #Save Tweets with lower case
            #print(attrH['text'])
            currTagList.append(attrH['text'].lower()) #Save Tweets with lower case
